...
const CVScoringPage = () => {
  ...
  const handleUploadFile = (file) => {
    ...
    if (file.type === "text/plain") {
      ...
      // For PDF/Word: user needs to paste text or use builder
      message.info(
        "File đã chọn. Với PDF/Word, hãy copy nội dung CV vào ô bên dưới để AI đọc chính xác hơn.",
      );
    }
    return false;
  };
  ...
  const scoreCV = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5000/cvs/score", {
        method: "POST",
        headers: {
      ...
      // Vì backend đã parse và trả về JSON chuẩn, frontend chỉ việc lấy ra dùng
